...
def _resolved_pairs(result: GraphResult) -> set[tuple[str, str]]:
    """(caller_qname, callee_qname) for every resolved internal call edge."""
...
def test_dead_function_has_no_incoming_edge() -> None:
    ...
    incoming = [e for e in result.edges if e.dst_id == helper.id]
